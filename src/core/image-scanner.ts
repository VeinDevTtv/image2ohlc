...
import { PlotAreaBounds, ContourDetectionParameters, UserClickCalibration, PixelCoordinates } from '../common/types';
 ...
 */
export async function findPlotArea(
  imagePath: string,
  params?: ContourDetectionParameters
): Promise<PlotAreaBounds> {
  if (!existsSync(imagePath)) {
    throw new Error(`Image file not found: ${imagePath}`);
  }
  ...
  try {
    ...
    const edgeImage = await sharp(grayImage)
      .convolve({
        width: 3,
        height: 3,
...
function traceContour(
  ...
  const contour: PixelCoordinates[] = [];
  const stack: PixelCoordinates[] = [{ x: startX, y: startY }];
 ...
 */
function calculateContourArea(contour: PixelCoordinates[]): number {
 ...
 */
function calculateAspectRatio(contour: PixelCoordinates[]): number {
 ...
 */
function contourToBounds(contour: PixelCoordinates[]): PlotAreaBounds {
  if (contour.length === 0) {
 ...
 */
function calculateContourConfidence(
  contour: PixelCoordinates[],
  imageWidth: number,
  imageHeight: number
): number {
  const bounds = contourToBounds(contour);
 ...
 */
export async function calibratePlotAreaManually(
  imagePath: string,
  calibration: UserClickCalibration
): Promise<PlotAreaBounds> {
  if (!existsSync(imagePath)) {
    throw new Error(`Image file not found: ${imagePath}`);
  }
