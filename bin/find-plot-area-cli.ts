...
import { findPlotArea, calibratePlotAreaManually, scanImage } from '../dist/core/image-scanner';
import { ContourDetectionParameters, UserClickCalibration } from '../dist/common/types';
...
function parseArguments(args: string[]): { imagePath: string; options: CLIOptions } {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Scan image metadata
    const scanResult = await scanImage(imagePath);
    ...
    if (options.manual) {
      ...
      const result = await calibratePlotAreaManually(imagePath, calibration);
      ...
      if (options.minArea !== undefined) params.minContourArea = options.minArea;
      if (options.maxArea !== undefined) params.maxContourArea = options.maxArea;
      ...
      const result = await findPlotArea(imagePath, params);
