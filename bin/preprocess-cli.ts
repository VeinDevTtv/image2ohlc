...
async function main(): Promise<void> {
  ...
  if (args.length === 0 || args.includes('--help') || args.includes('-h')) {
    ...
    console.log('');
    console.log('Usage:');
    console.log('  ts-node preprocess-cli.ts <image-path> [options]');
    console.log('');
    console.log('Options:');
    ...
    console.log('  ts-node preprocess-cli.ts chart.png');
    console.log('  ts-node preprocess-cli.ts --create-test-images --angle 3');
    console.log('  ts-node preprocess-cli.ts test.png --max-rotation 3');
    return;
  }
...
async function createTestImages(args: string[]): Promise<void> {
  ...
  const angleValue = angleIndex !== -1 ? args[angleIndex + 1] : undefined;
  const testAngle = angleValue !== undefined ? parseFloat(angleValue) : 2;
...
async function processImage(imagePath: string, args: string[]): Promise<void> {
  ...
  const maxRotationValue = maxRotationIndex !== -1 ? args[maxRotationIndex + 1] : undefined;
  const maxRotation = maxRotationValue !== undefined ? parseFloat(maxRotationValue) : 5;
